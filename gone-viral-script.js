    ...
    const SUPABASE_ANON_KEY = 'your-anon-key-here';  // Replace this with your actual anon key
    ...
    async function cancelBattle() {
      ...
      const { data: cancels } = await supabase.from("cancellations").select("*").eq("user_id", user.id);
      if (cancels.length >= 3) {
